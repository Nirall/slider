...
describe('Class Controller', () => {
  ...
  it('update() should change slider settings and reinit it', () => {
    ...
    expect([item.view.minValue, item.view.maxValue, item.view.step, item.view.isRange, item.view.isVertical, item.view.showLabel]).toEqual([1000, 10000, 100, true, true, true]);
  });

  it('update(), if data to update is not correct there is should not be a new property', () => {
    const item = new Controller();
    const entry = document.createElement('div');
    entry.classList.add('slider');
    document.body.appendChild(entry);
