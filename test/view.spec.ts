...
describe("View class", () => {
  ...
  it("init(), scaleFillingElem should has class 'slider__scale-filling_vertical' if vertical=true", () => {
    const newItem = new View(0, 800, 1, true, true);
    const entry = createElem("slider");
    document.body.appendChild(entry);
    newItem.append(entry);
    newItem.init();
    expect(newItem.scaleFilling.elem.classList.contains("slider__scale-filling_vertical")).toBeTrue();
  });

  it("init(), scaleFillingElem shouldn't has class 'slider__scale-filling_vertical' if vertical=false", () => {
    const newItem = new View(0, 800, 1, true, false);
    const entry = createElem("slider");
    document.body.appendChild(entry);
    newItem.append(entry);
    newItem.init();
    expect(newItem.scaleFilling.elem.classList.contains("slider__scale-filling_vertical")).toBeFalse();
  });
  ...
  it("init(), label1 elem should has class 'slider__button-label_vertical' if vertical=true", () => {
    const newItem = new View(0, 800, 1, true, true);
    const entry = createElem("slider");
    document.body.appendChild(entry);
    newItem.append(entry);
    newItem.init();
    expect(newItem.label1.elem.classList.contains("slider__button-label_vertical")).toBeTrue();
  });

  it("init(), label1 shouldn't has class 'slider__button-label_vertical' if vertical=false", () => {
    const newItem = new View(0, 800, 1, true, false);
    const entry = createElem("slider");
    document.body.appendChild(entry);
    newItem.append(entry);
    newItem.init();
    expect(newItem.label1.elem.classList.contains("slider__button-label_vertical")).toBeFalse();
  });

  it("init(), label2 elem should has class 'slider__button-label_vertical' if vertical=true", () => {
    const newItem = new View(0, 800, 1, true, true);
    const entry = createElem("slider");
    document.body.appendChild(entry);
    newItem.append(entry);
    newItem.init();
    expect(newItem.label2.elem.classList.contains("slider__button-label_vertical")).toBeTrue();
  });

  it("init(), label2 shouldn't has class 'slider__button-label_vertical' if vertical=false", () => {
    const newItem = new View(0, 800, 1, true, false);
    const entry = createElem("slider");
    document.body.appendChild(entry);
    newItem.append(entry);
    newItem.init();
    expect(newItem.label2.elem.classList.contains("slider__button-label_vertical")).toBeFalse();
  });
